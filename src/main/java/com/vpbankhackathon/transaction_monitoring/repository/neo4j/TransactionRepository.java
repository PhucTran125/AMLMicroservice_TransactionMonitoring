...
public interface TransactionRepository extends Neo4jRepository<Transaction, Long> {
    ...
    List<Transaction> findTransactionsBetweenAccounts(Long fromAccountId, Long toAccountId, LocalDateTime startTime, LocalDateTime endTime);
    ...
    @Query("CREATE (t:Transaction {id: $id, amount: $amount, date: $date, country: $country}) "
            + "WITH t "
            + "MATCH (from:Account {id: $fromAccountId}), (to:Account {id: $toAccountId}) "
            + "CREATE (t)-[:FROM_ACCOUNT]->(from), (t)-[:TO_ACCOUNT]->(to) "
            + "RETURN t")
    Transaction saveTransactionWithRelationships(Long id, Long amount, LocalDateTime date, String country, Long fromAccountId, Long toAccountId);
}
